...
def linkedList(data: Array[Double], start: Double, end: Double, dim: Double, clockwise: Boolean) = {
  var i = _
  var last: Node = _
...
def filterPoints(start: Node, end_par: Node = start) = {
  var end = end_par
  ...
  do {
    ...
    if (!p.steiner && (equals(p, p.next) || area(p.prev, p, p.next) == 0)) {
      removeNode(p)
      p = end = p.prev
      if (p == p.next) /* Unsupported: Break */ break;
      again = true
    } else {
      p = p.next
    }
...
def earcutLinked(ear_par: Node, triangles: Array[Double], dim: Double, minX: Double, minY: Double, invSize: Double, pass: Double) = {
  var ear = ear_par
  if (!ear) return
...
def isEarHashed(ear: Node, minX: Double, minY: Double, invSize: Double) = {
  val a = ear.prev
  val b = ear
  val c = ear.next
  if (area(a, b, c) >= 0) return false // reflex, can't be an ear
...
def cureLocalIntersections(start_par: Node, triangles: Array[Double], dim: Double) = {
  var start = start_par
  var p = start
  do {
    val a = p.prev
...
def splitEarcut(start: Node, triangles: Array[Double], dim: Double, minX: Double, minY: Double, invSize: Double) = {
...
def eliminateHoles(data: Array[Double], holeIndices: Array[Double], outerNode_par: Node, dim: Double) = {
  var outerNode = outerNode_par
  val queue = Array.empty[Node]
  var i = _
  var len = _
  var start: Double = _
  var end: Double = _
...
def eliminateHole(hole: Node, outerNode: Node) = {
...
def findHoleBridge(hole: Node, outerNode: Node) = {
  var p = outerNode
  val hx = hole.x
  val hy = hole.y
  var qx = -Infinity
...
def zOrder(x_par: Double, y_par: Double, minX: Double, minY: Double, invSize: Double) = {
  var x = x_par
  var y = y_par
...
def intersects(p1: Node, q1: Node, p2: Node, q2: Node) = {
...
def intersectsPolygon(a: Node, b: Node) = {
  var p = a
  do {
...
def insertNode(i: Double, x: Double, y: Double, last: Node) = {
...
class Node(var i: Double, var x: Double, var y: Double) {
  var prev: Node = null
  var next: Node = null
  var z = null
  var prevZ: Node = null
...
earcut.deviation = (data: Array[Double], holeIndices: Array[Double], dim: Double, triangles: Array[Double]) => {
  val hasHoles = holeIndices && holeIndices.length
  val outerLen = if (hasHoles) holeIndices(0) * dim else data.length
...
def signedArea(data: Array[Double], start: Double, end: Double, dim: Double) = {
  var sum = 0
  var i = start
  var j = end - dim
  while (i < end) {
...
earcut.flatten = (data: Array[Unit]) => {
